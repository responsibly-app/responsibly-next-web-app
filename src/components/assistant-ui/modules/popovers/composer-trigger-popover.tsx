            ...
            {items.map((item, index) => {
              ...
              return (
                <ComposerPrimitive.Unstable_TriggerPopoverItem
                  ...
                  item={item}
                  index={index}
                  className="flex w-full cursor-pointer flex-col items-start gap-0.5 px-3 py-2 rounded-xl text-start outline-none transition-colors hover:bg-accent focus:bg-accent data-highlighted:bg-accent"
                >
...
const ComposerTriggerPopoverImpl: FC<ComposerTriggerPopoverProps> = ({
  ...
  return (
    <ComposerPrimitive.Unstable_TriggerPopover
      ...
      className={cn(
        "aui-composer-trigger-popover absolute inset-s-0 bottom-full z-50 mb-2 w-64 overflow-hidden rounded-2xl border bg-popover text-popover-foreground shadow-lg",
        className,
      )}
      {...props}
    >
