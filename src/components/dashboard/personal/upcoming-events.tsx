...
import { useListAllUpcomingEvents, useGetRsvpStatus, useToggleRsvp } from "@/lib/auth/hooks";
...
type UpcomingEvent = ReturnType<typeof useListAllUpcomingEvents>["data"] extends
  | (infer T)[]
  | undefined
  ? T
  : never;

function UpcomingEventItem({ ev, userTz }: { ev: UpcomingEvent; userTz: string }) {
  const isRsvpable = ev.eventType === "in_person" || ev.eventType === "hybrid";
  const hasZoom = (ev.eventType === "online" || ev.eventType === "hybrid") && ev.zoomJoinUrl;

  const { data: rsvpStatus } = useGetRsvpStatus(isRsvpable ? ev.id : "");
  ...
  const showActions = hasZoom || isRsvpable;

  return (
    <li className="rounded-xl border overflow-hidden transition-colors hover:bg-muted/50">
      <Link href={routes.dashboard.eventDetail(ev.id)} className="flex flex-col gap-1.5 p-3">
      ...
      <CardContent>
        {isPending ? (
          ...
          <ul className="space-y-2">
            {events.map((ev) => (
              <UpcomingEventItem key={ev.id} ev={ev} userTz={userTz} />
            ))}
          </ul>
        )}
      </CardContent>
