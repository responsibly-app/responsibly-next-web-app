...
const STORAGE_KEY = "ama-monthly-goal";

// ── Hook ─────────────────────────────────────────────────────────────────────

export function useAmaGoal() {
...
function motivationText(current: number, goal: number): { text: string; good: boolean } {
  if (current === 0) {
    return { text: `${goal} recruit${goal !== 1 ? "s" : ""} to reach this month's goal`, good: false };
  }
...
// ── AmaGoalPopoverButton ──────────────────────────────────────────────────────

interface AmaGoalPopoverButtonProps {
  goal: number | null;
  setGoal: (value: number | null) => void;
}

export function AmaGoalPopoverButton({ goal, setGoal }: AmaGoalPopoverButtonProps) {
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-52 p-3" align="end">
        <div className="space-y-2.5">
          <Label className="text-xs font-medium">Monthly AMA goal</Label>
...
interface AmaGoalBarProps {
  current: number;
  goal: number;
}

export function AmaGoalBar({ current, goal }: AmaGoalBarProps) {
